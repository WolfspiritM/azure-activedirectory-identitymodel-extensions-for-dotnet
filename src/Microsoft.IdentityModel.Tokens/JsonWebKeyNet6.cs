...
namespace Microsoft.IdentityModel.Tokens
{
    ...
    /// </summary>
    internal class JsonWebKeyNet6 : SecurityKey
    {
        private string _kid;
        private const string _className = "Microsoft.IdentityModel.Tokens.JsonWebKeyNet6";

        /// <summary>
        /// Returns a new instance of <see cref="JsonWebKeyNet6"/>.
        /// </summary>
        /// <param name="json">A string that contains JSON Web Key parameters in JSON format.</param>
        /// <returns><see cref="JsonWebKeyNet6"/></returns>
        ...
        static public JsonWebKeyNet6 Create(string json)
        {
            if (string.IsNullOrEmpty(json))
                throw LogHelper.LogArgumentNullException(nameof(json));

            return new JsonWebKeyNet6(json);
        }

        /// <summary>
        /// Initializes an new instance of <see cref="JsonWebKeyNet6"/>.
        /// </summary>
        public JsonWebKeyNet6()
        {
        }

        /// <summary>
        /// Initializes an new instance of <see cref="JsonWebKeyNet6"/> from a json string.
        /// </summary>
        /// <param name="json">A string that contains JSON Web Key parameters in JSON format.</param>
        /// <exception cref="ArgumentNullException">If 'json' is null or empty.</exception>
        /// <exception cref="ArgumentException">If 'json' fails to deserialize.</exception>
        public JsonWebKeyNet6(string json)
        {
            if (string.IsNullOrEmpty(json))
                throw LogHelper.LogArgumentNullException(nameof(json));
            ...
            try
            {
                ...
                JsonWebKeyNet6Serializer.Read(json, this);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Gets the key id of this <see cref="JsonWebKeyNet6"/>.
        /// </summary>
        [JsonIgnore]
        ...
        {
            // TODO - only write if there are no values
            get;
            private set;
        } = new List<string>();
        ...
        internal string RepresentAsAsymmetricPublicJwk()
        {
            ...
            if (!string.IsNullOrEmpty(Kid))
                jsonObject[JsonWebKeyParameterNames.Kid] = Kid;

            if (string.Equals(Kty, JsonWebAlgorithmsKeyTypes.EllipticCurve))
                PopulateWithPublicEcParams(jsonObject);
            else if (string.Equals(Kty, JsonWebAlgorithmsKeyTypes.RSA))
                PopulateWithPublicRsaParams(jsonObject);
            else
        ...
        private void PopulateWithPublicEcParams(JsonObject jsonObject)
        {
            if (string.IsNullOrEmpty(Crv))
            ...
            jsonObject[JsonWebKeyParameterNames.Crv] = Crv;
            jsonObject[JsonWebKeyParameterNames.Kty] = Kty;
            jsonObject[JsonWebKeyParameterNames.X] = X;
            jsonObject[JsonWebKeyParameterNames.Y] = Y;
        }

        private void PopulateWithPublicRsaParams(JsonObject jsonObject)
        {
            if (string.IsNullOrEmpty(E))
            ...
            jsonObject[JsonWebKeyParameterNames.E] = E;
            jsonObject[JsonWebKeyParameterNames.Kty] = Kty;
            jsonObject[JsonWebKeyParameterNames.N] = N;
        }
